...
5. Нажать **🟢 Быстрая проверка**, затем **🔍 Полная диагностика**. Для рабочего
   ...
   проверенным типам.
6. Открыть **Telegram** в карточке аккаунта и проверить три способа:
   импорт авторизованного `.session`, вход по номеру и StringSession.
7. Для импорта убедиться, что файл проверяется до сохранения, становится
   `sessions/{account_id}.session`, а исходное сообщение удаляется из чата.
8. Для входа по номеру нажать **Запросить код**, проверить канал доставки,
   ввести код и при необходимости облачный пароль 2FA.
9. Для StringSession проверить удаление исходного сообщения и fallback:
   при существующем session-файле должен использоваться именно файл.
10. После каждого сценария открыть Health: ожидаются веса 20/10/20/20/30,
    время последней проверки и понятная причина ошибки при сбое.
11. Сохранить `DRY_RUN=True` и пройти симулятор/валидатор без реальной отправки.

## Проверка AccountOrchestrator

1. Открыть два-три аккаунта и почти одновременно запросить login code. Запросы
   должны выполниться через очередь, без запуска неограниченного числа клиентов.
2. Проверить состояния в карточке: `AUTHENTICATING` во время входа, `ACTIVE`
   после успешной авторизации, `DEGRADED` при отказе proxy и
   `REAUTH_REQUIRED` при недействительной сессии.
3. Сохранить proxy без проверки и запустить операцию аккаунта: orchestrator
   обязан сначала выполнить Telegram proxy check и не допустить Telethon к
   нерабочему proxy.
4. Убедиться, что реальные отправки, login, session import, fast diagnostics и
   background proxy monitor создают structured log с `account_id`, `proxy_id`,
   `action`, `status`, `latency_ms`.
5. Запустить `python -m unittest discover -s tests -v`: отдельные тесты проверяют
   лимит клиентов, login queue, health batches и preflight proxy validation.

## Проверка Autopilot

1. Убедиться, что после старта в логах есть `autopilot_started`, а цикл работает
   с интервалом `AUTOPILOT_INTERVAL_SECONDS` без Telegram-запроса сам по себе.
2. При 6 FloodWait за минуту ожидаются `DEGRADED`, пауза login queue и снижение
   client limit. При критическом уровне — один recovery slot и заморозка
   некритичных отправок.
3. При ошибках более чем у 10% аккаунтов ожидается `DEGRADED`; аккаунты получают
   cooldown, но не удаляются и их session-файлы не изменяются.
4. При более чем 30% failed proxy ожидается временная пауза proxy pool.
   Endpoint с failure rate выше 30% после минимум трёх проверок отключается на
   cooldown; его пароль нигде не выводится.
5. После двух стабильных циклов система проходит `RECOVERY` и возвращается в
   `HEALTHY`, login queue возобновляется, concurrency восстанавливается.

## Проверка lifecycle аккаунта

1. Отключить аккаунт: запись и `.session` должны сохраниться, scheduler больше
   не должен использовать аккаунт.
2. Запросить повторную авторизацию: текущая операция завершается, файл переходит
   в `sessions/reauth_archive/`, auth/proxy bindings очищаются, интерфейс ведёт
   к новому login flow.
3. Проверить, что старый код из FSM предыдущего поколения авторизации отклонён.
4. Попытаться удалить занятый аккаунт без `force`: ожидается безопасный отказ.
5. После остановки операций удалить аккаунт: запись, связанные чаты/logs и его
   эксклюзивный session-файл должны исчезнуть.
6. Проверить команды владельца `/account_disable ID`, `/account_reauth ID` и
   `/account_delete ID`, включая обязательное подтверждение и отказ оператору.
